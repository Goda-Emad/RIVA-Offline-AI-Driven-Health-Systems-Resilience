...
class AudioProcessor:
    ...
    def process(
        ...
        try:
            ...
            # 5. Capture pre-normalization noise floor for dynamic VAD threshold.
            #    Must happen BEFORE _normalize() — otherwise silence noise gets
            #    scaled up to the same level as speech and a fixed vad_threshold
            #    becomes unreliable after normalization.
            dynamic_vad_thr = self._compute_dynamic_vad_threshold(samples)

            # 6. Normalize amplitude
            if self.normalize:
                samples = self._normalize(samples)

            # 7. Trim leading/trailing silence (uses dynamic threshold)
            if self.trim_silence:
                samples = self._trim_silence(samples, vad_thr=dynamic_vad_thr)

            # 8. Build metadata & quality assessment (uses dynamic threshold)
            proc_ms = (time.perf_counter() - t_start) * 1000
            metadata = self._assess_quality(samples, fmt, proc_ms, dynamic_vad_thr)
    ...
    def process_stream(
        ...
        Fix: replaced np.concatenate(buffer, chunk) per-iteration with a
             list-based accumulator. np.concatenate copies the full array
             every call — O(N²) memory and CPU for long streams.
             Now we append decoded chunks to a list and concatenate once
             at flush time — O(N) total.

        Usage:
            for result in processor.process_stream(ws_chunks):
                transcript = whisper.transcribe(result.samples)
        """
        # Use a list to accumulate chunks — O(1) append, O(N) single concat
        ...
        for chunk in chunk_iterator:
            ...
            # VAD on the latest chunk only — no need to concatenate yet
            rms = self._rms(samples[-chunk_samples:] if len(samples) >= chunk_samples else samples)
            if rms < self.vad_threshold:
                silence_frames += 1
            else:
                silence_frames = 0

            # Flush when silence timeout reached
            if silence_frames >= silence_limit and buffer_len > chunk_samples:
                full_buffer = np.concatenate(buffer_chunks)   # single concat
                yield self.process(full_buffer, fmt=AudioFormat.FLOAT32)
                buffer_chunks = []
                buffer_len    = 0
                silence_frames = 0

        # Flush remaining audio
        if buffer_len > int(self.target_sr * MIN_AUDIO_DURATION_SEC):
            full_buffer = np.concatenate(buffer_chunks)
            yield self.process(full_buffer, fmt=AudioFormat.FLOAT32)
    ...
    def _trim_silence(
        ...
        frame_ms: int = 20,
        padding_ms: int = 100,
        vad_thr: Optional[float] = None,
    ) -> np.ndarray:
        """
        Trim leading/trailing silence using energy-based VAD.
        Keeps `padding_ms` of silence at each edge for natural speech.

        vad_thr: dynamic threshold from pre-normalization snapshot (preferred).
                 Falls back to self.vad_threshold if not supplied.
        """
        threshold = vad_thr if vad_thr is not None else self.vad_threshold
    ...
    def _compute_dynamic_vad_threshold(self, samples: np.ndarray) -> float:
        """
        Compute a dynamic VAD threshold from the pre-normalization signal.

        Strategy: estimate the noise floor from the quietest 20% of frames,
        then set threshold = noise_floor * 4.0  (6dB above noise).
        This is robust even when the overall signal level varies widely
        (e.g. a whisper vs. a shout — both get the right threshold).

        Called BEFORE _normalize() so the absolute energy levels are real.
        Falls back to self.vad_threshold if the signal is too short.
        """
        frame_len = int(self.target_sr * 0.02)          # 20ms frames
        if len(samples) < frame_len * 5:
            return self.vad_threshold                    # not enough frames

        energies: list[float] = sorted(
            float(self._rms(samples[i:i + frame_len]))
            for i in range(0, len(samples) - frame_len, frame_len)
        )
        noise_percentile = max(1, len(energies) // 5)
        noise_floor = float(np.mean(energies[:noise_percentile]))

        # 6dB above noise floor, clamped to [0.005, 0.2]
        dynamic = float(np.clip(noise_floor * 4.0, 0.005, 0.20))
        logger.debug(
            "Dynamic VAD threshold: %.4f  (noise_floor=%.4f, static=%.3f)",
            dynamic, noise_floor, self.vad_threshold,
        )
        return dynamic
    ...
    def _assess_quality(
        ...
        vad_thr: dynamic threshold computed pre-normalization (preferred).
                 Falls back to self.vad_threshold if not supplied.
        """
        thr      = vad_thr if vad_thr is not None else self.vad_threshold
        duration = len(samples) / self.target_sr
        rms      = float(self._rms(samples))
        snr_db   = self._estimate_snr(samples)

        # Speech ratio via frame-level VAD using the dynamic threshold
        frame_len = int(self.target_sr * 0.02)
        frames = [
            self._rms(samples[i:i + frame_len]) > thr
            for i in range(0, len(samples) - frame_len, frame_len)
        ]
    ...
    def _estimate_snr(self, samples: np.ndarray) -> float:
        ...
        Fix: removed @staticmethod — called via self._estimate_snr(samples)
             so self must be the first argument.
        Fix: explicit float() casts before math.log10 to avoid np.float64
             edge-cases in older numpy versions.
        """
        if len(samples) < 320:
            return 0.0
        ...
        frame_len = 160
        energies: list[float] = sorted(
            float(np.mean(samples[i:i + frame_len] ** 2))
            for i in range(0, len(samples) - frame_len, frame_len)
        )
        ...
        noise_idx   = max(1, len(energies) // 10)
        signal_idx  = len(energies) // 2

        # explicit float() prevents np.float64 from causing math.log10 issues
        noise_floor = float(max(float(np.mean(energies[:noise_idx])),  1e-12))
        signal_pwr  = float(max(float(np.mean(energies[signal_idx:])), 1e-12))

        snr = 10.0 * math.log10(signal_pwr / noise_floor)
