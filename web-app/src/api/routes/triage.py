...
🔐 متكامل مع نظام التحكم بالصلاحيات (Depends Only)
...
async def verify_system_role(
    ...
    allowed_services = ["sentiment_analyzer", "prediction_engine", "orchestrator"]
...
class EmergencySignal(BaseModel):
    ...
    service_signature: Optional[str] = Field(None)
...
class EmergencySignalHandler:
    ...
    @classmethod
    async def handle_emergency_signal(cls, signal: EmergencySignal, source_service: str) -> Dict:
        logger.warning(
...
async def predict_triage(
    ...
    # ✅ التعديل: require_any_role كـ Depends مش decorator
    _auth=Depends(require_any_role([Role.DOCTOR, Role.NURSE, Role.ADMIN, Role.SUPERVISOR]))
):
    """🚑 تصنيف حالة المريض حسب درجة الطوارئ"""
    start_time = datetime.now()
    request_id = hashlib.md5(
        f"{data.patient_id or 'unknown'}_{datetime.now().isoformat()}".encode()
    ).hexdigest()[:16]
...
async def receive_emergency_signal(
    ...
    if signal.emergency_level not in ["high", "critical"]:
        raise HTTPException(status_code=400, detail=f"Invalid emergency level: {signal.emergency_level}")

    if not signal.emergency_keywords:
        raise HTTPException(status_code=400, detail="Emergency keywords cannot be empty")
...
async def _emergency_triage(signal: EmergencySignal, source_service: str):
    try:
        ...
        features = {
            ...
            "DiabetesPedigreeFunction": 0, "Age": demographics.get('age', 0)
        }
...
# =========================================================================
# 🔒 Security Audit Endpoint — ✅ تم التعديل: Depends بدل decorator
# =========================================================================

@router.get("/security-audit")
async def get_security_audit_logs(
    # ✅ التعديل الرئيسي: require_role كـ Depends مش @decorator
    _auth=Depends(require_role(Role.ADMIN))
):
...
# =========================================================================
# Utility Endpoints — ✅ تم التعديل: Depends بدل decorator
# =========================================================================

@router.get("/samples", response_model=dict)
async def get_samples(
    # ✅ التعديل
    _auth=Depends(require_any_role([Role.DOCTOR, Role.NURSE, Role.ADMIN]))
):
    try:
...
async def get_features(
    # ✅ التعديل
    _auth=Depends(require_any_role([Role.DOCTOR, Role.ADMIN]))
):
    try:
